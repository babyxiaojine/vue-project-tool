    ...
    <div class="calendar-header">
      <div class="calendar-info" :style="{'background-color': options.header_bg_color}">
        <transition name="fade">
          <div :style="{'color': options.header_font_color}" class="head-year">
            <div @click="preYear" class="left-year-content">
              <i class=" wm-icon left-arr left-little" ></i>
              <i class=" wm-icon left-arr" ></i>
            </div>

            <div @click="preMonth" class="left-month-content">
              <i class=" wm-icon left-arr" ></i>
            </div>

           <div class="middle-year-content">{{ nowDay }}</div>

            <div @click="nextMonth" class="right-month-content">
              <i class="wm-icon right-arr" ></i>
            </div>

            <div @click="nextYear" class="right-year-content">
              <i class="wm-icon right-arr" ></i>
              <i class="wm-icon right-arr right-little" ></i>
            </div>
      ...
      <ul>
        <li v-for="week in weeks" :key="week" class="week" :style="{'color': options.week_font_color}">{{week}}</li>
      </ul>
    </div>

    <div class="calendar-box" :id="boxId">
      <div class="calendar-wrapper" :id="wrapperId">
        <table cellpadding="5" class="last-month">
          <tbody>
          <tr v-for="(day,k1) in lastDays" :key="k1" :style="{animationDelay:'('+k1*30+')ms'}">
            <td v-for="(child,k2) in day" :key="k2" :class="{'selected':child.selected,'disabled':child.disabled}" :style="{height: tdH + 'px'}">
              ...
              <span class="dot dotBlue" v-if="child.eventBlueName"></span>
            </td>
          </tr>
          </tbody>
        </table>
        <table cellpadding="5" class="now-month">
          <tbody>
          <tr v-for="(day,k1) in days" :key="k1" :style="{animationDelay:'('+k1*30+')ms'}">
        ...
        <table cellpadding="5" class="now-month">
          <tbody>
          <tr v-for="(day,k1) in nextDays" :key="k1" :style="{animationDelay:'('+k1*30+')ms'}">
            <td v-for="(child,k2) in day" :key="k2" :class="{'selected':child.selected,'disabled':child.disabled}" :style="{height: tdH + 'px'}">
              <span :style="dateStyle(child)">{{child.day}}</span>
              <span class="dot" v-if="child.eventName"></span>
              <span class="dot dotBlue" v-if="child.eventBlueName"></span>
    ...
    props: {
      options: {
        ...
        default: function () {
          return {
            header_bg_color: '#fff',
            header_font_color: '#7C86A2',
            week_font_color: 'rgba(51,72,94,0.54)',
            selected_bg_color: '#4196F7',
...
//      this.calendarBox = document.getElementsByClassName('calendar-box')[0]
      this.calendarBox = document.getElementById(this.boxId)
//      this.wrapper = document.getElementsByClassName('wrapper')[0]
      this.wrapper = document.getElementById(this.wrapperId)
      this.init()
      this.tdH = Math.round((document.documentElement.clientWidth || document.body.clientWidth) / 10)
    },
    methods: {
      init() {
        let now = new Date();
        ...
        this.$emit('toDay', [this.year, this.zero ? this.zeroPad(this.month + 1) : this.month + 1, this.zero ? this.zeroPad(this.day) : this.day])
        let weekIndex = now.getDay();
        let weekValue = this.weeks[weekIndex];
        this.$emit('week',weekValue);
        //  listener touch event
        this.wrapper.style.transform = 'translateX(' + -(document.documentElement.clientWidth || document.body.clientWidth) + 'px' + ')'
        ...
        for (i = 1; i <= lastDateOfMonth; i++) {
          ...
          if (day == 0) {
            ...
            for (let j = 0; j < firstDayOfMonth; j++) {
              temp[line].push(Object.assign(
                ...
                isCurrentMonth ? this.getEventsBlue(this.computedPrevYear(),this.computedPrevMonth(true),k) : {},
          ...
          if (this.selectedDate.length == 3 && y == this.selectedDate[0] && m == this.selectedDate[1] &&  i == this.selectedDate[2]) {
            temp[line].push(Object.assign(
              ...
              isCurrentMonth ? this.getEventsBlue(y, m, i) : {},
              this.isToday(y, m, i)
            ))
          ...
          if (day == 6 && i < lastDateOfMonth) {
            ...
            for (let d=day; d < 6; d++) {
              temp[line].push(Object.assign(
                ...
                isCurrentMonth ? this.getEventsBlue(this.computedNextYear(), this.computedNextMonth(true), k) : {},
          ...
          for (let i = line+1; i<=5; i++) {
            ...
            for (let d=start; d <= start+6; d++) {
              temp[i].push(Object.assign(
                ...
                isCurrentMonth ? this.getEventsBlue(this.computedNextYear(),this.computedNextMonth(true),d) : {},
                this.isToday(this.computedNextYear(), this.computedNextMonth(true) + 1, d)
              ))
            }
          }
        ...
        if(this.isScrolling === 0) { //当为水平滚动时
          if (Number(duration) < 300 && Math.abs(this.endPos.x) > 30) {     //  快速拖拽并松开
            if (this.endPos.x > 30) {
              ...
              this.wrapper.style.transition = 'all 300ms ease'
              this.wrapper.style.transform = 'translateX(' + -(document.documentElement.clientWidth || document.body.clientWidth) * 2 + 'px)'
              this.resetTransition('right')
            }
          } else if (Number(duration) >= 300) {    //  持续拖拽
            if (Math.abs(this.endPos.x) > (document.documentElement.clientWidth || document.body.clientWidth) / 2) {
              if (this.endPos.x > 0) {
                this.wrapper.style.transition = 'all 300ms ease'
                this.wrapper.style.transform = 'translateX(' + 0 + 'px)'
                this.resetTransition('left')
      ...
      // 获取自定义事件
      getEventsBlue(y, m, d) {
        if (this.eventsBlue.length === 0) return false
        let isContain = false
        this.eventsBlue.forEach(item => {
          let days = item.split('-')
          if (days.length == 3 && days[0] == y && Number(days[1]) == (m + 1) && Number(days[2] == d)) {
            isContain = true
          }
      ...
      select(k1, k2, e) {
        ...
        let selectYear = this.year.toString();
        let selectMon = this.zero ? this.zeroPad(this.month + 1) : this.month + 1;
        let selectDay = this.zero ? this.zeroPad(this.days[k1][k2].day) : this.days[k1][k2].day;
        let fullDate = selectYear+'/'+selectMon+'/'+selectDay;
        this.$emit('select', {
          formatTime: new Date(fullDate).getTime(),
          arrayTime:[selectYear, selectMon, selectDay]
        });
        //获取星期几
        const dateStr = this.year + "-" + this.zeroPad(this.month + 1) + "-" + this.zeroPad(this.days[k1][k2].day);
        const myDate = new Date(Date.parse(dateStr.replace(/-/g, "/"))).getDay();
        const weekValue = this.weeks[myDate];
        this.$emit('week',weekValue);
      },
      // 返回今天
      setToday() {
...
<style scoped>
  ...
  .calendar > .calendar-box > .calendar-wrapper {
    display: -webkit-box;
    width: 300%;
  ...
  .left-year-content{display: inline-block;width: 16px;height: 20px;position: relative;left: 0}
  .left-month-content{display: inline-block;width: 10px;height: 20px;position: relative;left: 0;margin: 0 20px}
  .right-year-content{display: inline-block;width: 16px;height: 20px;position: relative;left: 0}
  .right-month-content{display: inline-block;width: 16px;height: 20px;position: relative;left: 0;margin: 0 20px}
  .middle-year-content{display: inline-block;width: 120px;height: 20px;vertical-align: top}
  .head-year .left-arr{left: 0}
  .head-year .left-little{left: 60%}
  .head-year .right-arr{left: 60%}
  .head-year .right-little{left:0}
</style>
