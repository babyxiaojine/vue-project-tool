<template>
    <!--CHECK1-->
    <div class="wrapper add-month" v-if="objData.taskDefinitionKey == 'check1'&&!objData.readonly">
        ...
        <form ref="postForm" class="post-form">
            <monthPlanItem ref="getMonthItem" :pageParam="saveData" :followup.sync="followUp" :readonly="readonly" @itemData="getItemData">
    ...
    <!--CHECK2-->
    <div class="wrapper month-detail scroll-layout" v-else-if="objData.taskDefinitionKey == 'check2' || (objData.taskDefinitionKey == 'check1'&&objData.readonly)">
...
<script type="text/ecmascript-6">
  ...
  import monthPlanDetail from '@/pages/plan/staff/children/monthPlanDetail'
  import monthPlanItem from '@/pages/plan/staff/children/monthPlanItem'
  ...
  export default {
    ...
    mounted(){
      const {taskDefinitionKey,readonly} = this.objData;
      let {yearAndMonth,level2} = this.detailData;
      //获取计划数据
      this.getPlanDetail().then(()=>{
        if (taskDefinitionKey == 'check1'&&!readonly) this.transPlanItem();
      });
      if (!level2) level2 = this.userBase.level2;
      //获取开发部
    ...
    methods: {
      ...
      saveMonthPlan(){
        ...
        postTokenData(base.basePath + '/expandshop/devPlan/devPlanMonth/devAttache/commitAudit',postParam, {
          success: (res)=> {
            if (res.success) {
              this.$router.back(-1);
            } else {
      ...
      confirmComment(comment,actionType){
        ...
        postTokenData('/expandshop/devPlan/devPlanMonth/devAttache/audit',sectionData,(res)=>{
          if(res.code==='0000'){
