                ...
                <ul class="clearfix">
                    <li v-for="(item,index) in reqObjPointTraceParams" :key="index">
                        <div class="tl-dir bdtop">
                            <div class="td-title">
                                <em>{{item.objName || item.shopName}}<i v-if="item.isInMonth == 1" class="within-plan"></i></em>
                                <span>编号：{{item.code}}</span>
                            </div>
                            <span>{{item.devName}}</span>
                            <button v-if="!readonly && item.editFlag" type="button" @click="delAppointItem({index:index,filed:'reqObjPointTraceParams',tag:item.code})">删除</button>
                        </div>
                        <div class="tl-cont row-list bdtb">
                            ...
                            <div class="row-list-item" @click="item.editFlag && selectItemOption({index:index,filed:'reqObjPointTraceParams',name:'currentMonthObjStatus'})">
                                <div class="cell-title">本月目标状态</div>
                                <div class="cell-value">
                                    <input class="ipt-data" type="text" :value="item.currentMonthObjStatusName" rule="required" error="请选择本月目标状态" placeholder="请选择" readonly><i v-if="!readonly && item.editFlag" class="iconfont icon-right-arrow"></i>
                                </div>
                            </div>
                            <div class="row-list-item" @click="item.editFlag && selectDateTime({index:index,filed:'reqObjPointTraceParams',name:'predictDecidedTime'})">
                                <div class="cell-title">预计定店时间</div>
                                <div class="cell-value">
                                    <input class="ipt-data" type="text" :value="formatDate(item.predictDecidedTime,'yyyy-MM-dd')" rule="required" error="请选择预计定店时间" placeholder="请选择" readonly><i v-if="!readonly && item.editFlag" class="iconfont icon-right-arrow"></i>
                                </div>
                            </div>
                            ...
                            <div class="row-list-item" @click="item.editFlag && selectItemOption({index:index,filed:'reqObjPointTraceParams',name:'shopDecidedType'})">
                                <div class="cell-title">定店类型</div>
                                <div class="cell-value">
                                    <input class="ipt-data" type="text" :value="item.shopDecidedType && (item.shopDecidedType == '1'?'新店':'扩店')" rule="required" error="请选择定店类型" placeholder="请选择" readonly><i v-if="!readonly && item.editFlag" class="iconfont icon-right-arrow"></i>
                                </div>
                            </div>
                            <div class="row-list-item" @click="item.editFlag && selectDateTime({index:index,filed:'reqObjPointTraceParams',name:'predictOpenTime'})">
                                <div class="cell-title">预计开店时间</div>
                                <div class="cell-value">
                                    <input class="ipt-data" type="text" :value="formatDate(item.predictOpenTime,'yyyy-MM-dd')" rule="required" error="请选择预计开店时间" placeholder="请选择" readonly><i v-if="!readonly && item.editFlag" class="iconfont icon-right-arrow"></i>
                                </div>
                            </div>
            ...
            <div class="target-list" v-show="tabIndex == 1">
                <ul class="clearfix">
                    <li v-for="(item,index) in reqNewShopOpenParams" :key="index">
                        <div class="tl-dir bdtop">
                            <div class="td-title">
                                <em>{{item.shopName}}<i v-if="item.isInMonth == 1" class="within-plan"></i></em>
                                <span>编号：{{item.code}}</span>
                            </div>
                            <span>{{item.devName}}</span>
                            <button v-if="!readonly && item.editFlag" type="button" @click="delAppointItem({index:index,filed:'reqNewShopOpenParams',tag:item.code})">删除</button>
                        </div>
                        <div class="tl-cont row-list bdtb">
                            ...
                            <div class="row-list-item" @click="item.editFlag && selectDateTime({index:index,filed:'reqNewShopOpenParams',name:'predictOpenTime'})">
                                <div class="cell-title">预计开业时间</div>
                                <div class="cell-value">
                                    <input class="ipt-data" type="text" :value="formatDate(item.predictOpenTime,'yyyy-MM-dd')" rule="required" error="请选择预计开业时间" placeholder="请选择" readonly><i v-if="!readonly && item.editFlag" class="iconfont icon-right-arrow"></i>
                                </div>
                            </div>
                        </div>
                    </li>
                ...
                <ul class="clearfix">
                    <li v-for="(item,index) in reqExtendShopOpenParams" :key="index">
                        <div class="tl-dir bdtop">
                            ...
                            <span>{{item.devName}}</span>
                            <button v-if="!readonly && item.editFlag" type="button" @click="delAppointItem({index:index,filed:'reqExtendShopOpenParams',tag:item.code})">删除</button>
                        </div>
                        <div class="tl-cont row-list bdtb">
                            <div class="row-list-item"><div class="cell-title">门店店号</div><div class="cell-value">{{item.code}}</div></div>
                            <div class="row-list-item"><div class="cell-title">工程进度</div><div class="cell-value">{{item.projectProgressName}}</div></div>
                            <div class="row-list-item" @click="item.editFlag && selectDateTime({index:index,filed:'reqExtendShopOpenParams',name:'predictOpenTime'})">
                                <div class="cell-title">预计开业时间</div>
                                <div class="cell-value">
                                    <input class="ipt-data" type="text" :value="formatDate(item.predictOpenTime,'yyyy-MM-dd')" rule="required" error="请选择预计开业时间" placeholder="请选择" readonly><i v-if="!readonly && item.editFlag" class="iconfont icon-right-arrow"></i>
                                </div>
                ...
                <ul class="clearfix">
                    <li v-for="(item,index) in reqReNewShopParams" :key="index">
                        <div class="tl-dir bdtop">
                            ...
                            <span>{{item.devName}}</span>
                            <button v-if="!readonly && item.editFlag" type="button" @click="delAppointItem({index:index,filed:'reqReNewShopParams',tag:item.code})">删除</button>
                        </div>
                        <div class="tl-cont row-list bdtb">
                            <div class="row-list-item"><div class="cell-title">门店店号</div><div class="cell-value">{{item.code}}</div></div>
                            <div class="row-list-item" @click="item.editFlag && selectDateTime({index:index,filed:'reqReNewShopParams',name:'predictRenewTime'})">
                                <div class="cell-title">预计续签时间</div>
                                <div class="cell-value">
                                    <input class="ipt-data" type="text" :value="formatDate(item.predictRenewTime,'yyyy-MM-dd')" rule="required" error="请选择预计续签时间" placeholder="请选择" readonly><i v-if="!readonly && item.editFlag" class="iconfont icon-right-arrow"></i>
                                </div>
                            </div>
                            <div class="row-list-item" @click="item.editFlag && selectDateTime({index:index,filed:'reqReNewShopParams',name:'contractExpirationTime'})">
                                <div class="cell-title">合同到期时间</div>
                                <div class="cell-value">
                                    <input class="ipt-data" type="text" :value="formatDate(item.contractExpirationTime,'yyyy-MM-dd')" rule="required" error="请选择合同到期时间" placeholder="请选择" readonly><i v-if="!readonly && item.editFlag" class="iconfont icon-right-arrow"></i>
                                </div>
                            </div>
                ...
                <ul class="clearfix">
                    <li v-for="(item,index) in reqCloseShopParams" :key="index">
                        <div class="tl-dir bdtop">
                            ...
                            <span>{{item.devName}}</span>
                            <button v-if="!readonly && item.editFlag" type="button" @click="delAppointItem({index:index,filed:'reqCloseShopParams',tag:item.code})">删除</button>
                        </div>
                        <div class="tl-cont row-list bdtb">
                            <div class="row-list-item"><div class="cell-title">门店店号</div><div class="cell-value">{{item.code}}</div></div>
                            <div class="row-list-item"><div class="cell-title">门店性质</div><div class="cell-value">{{item.shopNature == '1'?'自营店':'加盟店'}}</div></div>
                            <div class="row-list-item" @click="item.editFlag && selectDateTime({index:index,filed:'reqCloseShopParams',name:'predictCloseTime'})">
                                <div class="cell-title">预计闭店时间</div>
                                <div class="cell-value">
                                    <input class="ipt-data" type="text" :value="formatDate(item.predictCloseTime,'yyyy-MM-dd')" rule="required" error="请选择预计闭店时间" placeholder="请选择" readonly><i v-if="!readonly && item.editFlag" class="iconfont icon-right-arrow"></i>
                                </div>
                            </div>
                            <div class="row-list-item" @click="item.editFlag && selectDateTime({index:index,filed:'reqCloseShopParams',name:'contractExpirationTime'})">
                                <div class="cell-title">合同到期时间</div>
                                <div class="cell-value">
                                    <input class="ipt-data" type="text" :value="formatDate(item.contractExpirationTime,'yyyy-MM-dd')" rule="required" error="请选择合同到期时间" placeholder="请选择" readonly><i v-if="!readonly && item.editFlag" class="iconfont icon-right-arrow"></i>
                                </div>
                            </div>
                            <div class="row-list-item" @click="item.editFlag && selectItemOption({index:index,filed:'reqCloseShopParams',name:'budgetFlag'})">
                                <div class="cell-title">预算内外</div>
                                <div class="cell-value">
                                    <input class="ipt-data" type="text" :value="item.budgetFlag && (item.budgetFlag == '1'?'预算内':'预算外')" rule="required" error="请选择预算内外" placeholder="请选择" readonly><i v-if="!readonly && item.editFlag" class="iconfont icon-right-arrow"></i>
                                </div>
                            </div>
                            <div class="row-list-item" @click="item.editFlag && selectItemOption({index:index,filed:'reqCloseShopParams',name:'closeReason'})">
                                <div class="cell-title">闭店原因</div>
                                <div class="cell-value">
                                    <input class="ipt-data" type="text" :value="item.closeReasonName" rule="required" error="请选择闭店原因" placeholder="请选择" readonly><i v-if="!readonly && item.editFlag" class="iconfont icon-right-arrow"></i>
                                </div>
                            </div>
                        </div>
                    </li>
            ...
            <div class="target-list" v-show="tabIndex == 5">
                <ul class="clearfix">
                    <li v-for="(item,index) in reqBrandAddParams" :key="index">
                        <div class="tl-dir bdtop">
                            <div class="td-title">
                                <em>{{item.brandName}}<i v-if="item.isInMonth == 1" class="within-plan"></i></em>
                                <span>编号：{{item.code}}</span>
                            </div>
                            <button v-if="!readonly && item.editFlag" type="button" @click="delAppointItem({index:index,filed:'reqBrandAddParams',tag:item.code})">删除</button>
                        </div>
                        <div class="tl-cont row-list bdtb">
                            <div class="row-list-item" @click="item.editFlag && selectItemOption({index:index,filed:'reqBrandAddParams',name:'projectName',brandName:item.brandName,projectName:item.projectName})">
                                <div class="cell-title">项目名称</div>
                                <div class="cell-value">
                                    <input class="ipt-data" type="text" :value="item.projectName" rule="required" error="请选择项目名称" placeholder="请选择" readonly><i v-if="!readonly && item.editFlag" class="iconfont icon-right-arrow"></i>
                                </div>
                            </div>
                            ...
                            <div class="row-list-item" @click="item.editFlag && selectDateTime({index:index,filed:'reqBrandAddParams',name:'predictVisitTime'})">
                                <div class="cell-title">预计拜访时间</div>
                                <div class="cell-value">
                                    <input class="ipt-data" type="text" :value="formatDate(item.predictVisitTime,'yyyy-MM-dd')" rule="required" error="请选择预计拜访时间" placeholder="请选择" readonly><i v-if="!readonly && item.editFlag" class="iconfont icon-right-arrow"></i>
                                </div>
                            </div>
    ...
    export default {
        ...
        data () {
            return {
                ...
                planType: [
                    ...
                    {type:'newShopOpen',name:'新店开业'},
                    {type:'extendShopOpen',name:'扩店开业'},
        ...
        methods: {
            ...
            addPlanItem(){
                ...
                let {startDate,level3} = this.pageParam;
            ...
            //周计划是否在月计划中
            checkInMonthPlan(otherId){
//                console.log('checkInMonthPlan otherId = '+otherId);
//                if(!this.checkInfo || !this.checkInfo.year || !this.checkInfo.year.month){
//                    this.checkInfo = {
//
//                    }
//                }
                this.checkInfo.otherId = otherId;
                return new Promise((resolve, reject)=>{
                    postTokenData(base.basePath + '/devPlan/devPlanWeek/manager/checkInMonthPlan',this.checkInfo, (res)=>{
                        if (res.success) {
                            resolve(res.data);
                        } else {
            ...
            changeMonthItem(val,type){
                if(type != 'brand'){
                    this.checkInMonthPlan(val.id).then(isInMonth => {
                        this.rendItemData(isInMonth,val,type);
                    })
                }else{
                    this.rendItemData(0,val,type);
                }
            },
            rendItemData(isInMonth,val,type){
                let itemObj = addType[type];
                itemObj.editFlag = true;
                itemObj.isInMonth = isInMonth;
                itemObj.inMonthFlag = isInMonth?'1':'0';
                ...
                if (type === 'objPoint' || type === 'objTrace'){
                    itemObj = {
                        ...itemObj,
                        code: val.formalCode,
                        objName: val.name,
                ...
                if (type === 'newShopOpen' || type === 'extendShopOpen'){
                    ...
                    if (type === 'extendShopOpen') filedName = 'reqExtendShopOpenParams';
                    //添加
            ...
            selectItemOption(selectParam){
                ...
                if (selectParam.name == 'currentMonthObjStatus'){
                    ...
                    getPlanSelect('brandName',{brandName:selectParam.brandName,projectName:selectParam.projectName}).then((res)=>{
            ...
            changeItemOption(val,param){
                ...
                console.log('changeItemOption iName = '+iName);
                ...
                if (iName == 'projectName'){//品牌信息
                    ...
                    getPlanSelect('brandInfo',{brandName:brand,projectName:val}).then((res)=>{
                        ...
                        console.log('changeItemOption >>>>>');

                        this.checkInMonthPlan(res.id).then(isInMonth => {
                            ...
                            this[iFiled][iIndex].editFlag = true;
                            this[iFiled][iIndex].isInMonth = isInMonth;
                            this[iFiled][iIndex].inMonthFlag = isInMonth?'1':'0';
                            this.$set(thisObj,'brandStatusName',res.statusName);

                            console.log('changeItemOption 《《《《《《');
                            console.log(this[iFiled][iIndex]);
                        })
                    })
            ...
            selectTimeConfirm(val){
                ...
                let iFiled = tType.filed;
                let iIndex = tType.index;
                let iName = tType.name;
                if (val){
                    ...
                    this[iFiled][iIndex][iName] = formatTime;
