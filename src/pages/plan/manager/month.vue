<template>
    <div class="wrapper staff-month">
        ...
        <div class="month-list">
            <ul class="approach-area" v-if="listData.length">
                <li class="approach-item" v-for="(item,index) in listData" :key="index">
                    <router-link :to="{name: 'ManagerMonthDetail', params: item}">
        ...
        <div class="foot-submit" v-if="pageAuth.add">
            <router-link tag="div" class="form-handles fix-area" :to="{name: 'planManagerAddMonth'}">
  ...
  export default {
    ...
    methods: {
        setAuthUser(){
            ...
            if(AuthorityInfo.indexOf('devManageMonthPlan:add')>-1){
                pageAuth.add = true;
            }
        ...
        getDataInfo(type,year){
            let { auditType } = this;
            postTokenData('/devPlan/devPlanMonth/devManagerMonthList',{auditType:type,year},(res)=>{
                if(res.success){
        ...
        delMonthPlan(item){
            ...
            MessageBox.confirm('确定删除此项！','提示',{
                ...
                postTokenData('/devPlan/delete',postParam,(res)=>{
                    if(res.success){
