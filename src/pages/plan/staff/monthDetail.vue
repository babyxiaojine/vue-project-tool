...
<script type="text/ecmascript-6">
  ...
  import monthPlanDetail from './children/monthPlanDetail'
  import HistoryFlow from '@/pages/task/historyFlow'
  export default {
    name: 'StaffMonthDetail',
    data() {
      return {
        pageParam:{},//页面传参
    ...
    created() {
      ...
      const sessionParam = getSession('StaffMonthDetail');
      if (routeParams.id){
        this.pageParam = routeParams;
      } else if (sessionParam.id){
        this.pageParam = sessionParam;
      }
      // console.log(routeParams,sessionParam,'333333333333333333')
    },
    mounted(){
    ...
    methods: {
      ...
      getPlanDetail(planType){
        ...
        postTokenData(base.basePath + '/expandshop/devPlan/devPlanMonth/devAttache/detail',postData, (res)=>{
          if (res.success) {
